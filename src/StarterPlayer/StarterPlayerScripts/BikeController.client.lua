...
local function resetWheelie(controllerState)
	...
	controllerState.WheelieQueuedUntil = 0
	...
	controllerState.ThrottleHeld = false
end
...
local function applyTraversalAssist(controllerState, groundResult, look, dt)
	...
	if controllerState.WheelieActive and math.max(controllerState.WheeliePitch, controllerState.WheelieTargetPitch) >= Config.Bike.Wheelie.TraversalAssistSuppressAngle then
		return
	end
...
local function hasForwardIntent(controllerState, throttle)
	if UserInputService:GetFocusedTextBox() then
		return false
	end

	local seatThrottle = throttle
	if seatThrottle == nil and controllerState.Seat then
		seatThrottle = controllerState.Seat.ThrottleFloat
	end

	if seatThrottle and seatThrottle > 0 then
		return true
	end

	return controllerState.ThrottleHeld
end

local function queueWheelieRequest(controllerState, nowStamp)
	local wheelieConfig = Config.Bike.Wheelie
	local requestUntil = math.max(controllerState.WheelieArmedUntil, nowStamp + wheelieConfig.RequestWindow)
	controllerState.WheelieQueuedUntil = math.max(controllerState.WheelieQueuedUntil, requestUntil)
end
...
local function tryStartWheelie(controllerState, grounded, throttle, look, topSpeed)
	...
	if controllerState.WheelieArmedUntil > 0 and nowStamp > controllerState.WheelieArmedUntil then
		controllerState.WheelieArmedUntil = 0
	end
	if controllerState.WheelieQueuedUntil > 0 and nowStamp > controllerState.WheelieQueuedUntil then
		controllerState.WheelieQueuedUntil = 0
	end
	if controllerState.WheelieQueuedUntil <= 0 then
		return
	end
	...
	if not grounded or throttle < 0 or speedMph < wheelieConfig.MinSpeedMph or not controllerState.Hull or not hasForwardIntent(controllerState, throttle) then
		return
	end
...
local function updateWheelie(controllerState, grounded, throttle, look, topSpeed, dt)
	local wheelieConfig = Config.Bike.Wheelie
	local forwardIntent = hasForwardIntent(controllerState, throttle)
	...
	if controllerState.WheelieActive then
		if speedMph < wheelieConfig.SustainSpeedMph or throttle < 0 then
			...
			local minTarget = forwardIntent and wheelieConfig.SustainAngle or 0
...
UserInputService.InputBegan:Connect(function(input, gameProcessed)
	...
	elseif input.KeyCode == Enum.KeyCode.LeftControl or input.KeyCode == Enum.KeyCode.RightControl then
		local nowStamp = currentTime()
		controller.WheelieArmedUntil = nowStamp + Config.Bike.Wheelie.ArmWindow
		if controller.BikeModel and controller.BikeDef and hasForwardIntent(controller) then
			queueWheelieRequest(controller, nowStamp)
		end
	elseif input.KeyCode == Enum.KeyCode.W then
		controller.ThrottleHeld = true
		local nowStamp = currentTime()
